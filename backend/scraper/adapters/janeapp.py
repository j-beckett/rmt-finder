...
class JaneAppAdapter(BaseAdapter):
    ...
    def _fetch_router_options(self, clinic) -> dict:
        ...
        try:
            ...
            fixed = re.sub(r'([{,]\s*)([a-zA-Z_$][a-zA-Z0-9_$]*)(\s*:)', r'\1"\2"\3', decoded)
    ...
    def _map_services(self, clinic, disciplines: list, treatments: list) -> dict:
        """
        For each service in the clinic config, find the matching
        Jane discipline and treatment IDs.
        """
        service_map = {}
    ...
    def _fetch_openings(self, clinic, service_type, treatment, staff_lookup) -> list[AvailabilityResult]:
        ...
        if openings:
            print(f"    treatment {treatment['treatment_id']}: {len(openings)} openings, first start_at={openings[0].get('start_at')}")
    ...
    def discover(self, clinic) -> dict:
        ...
        print(f"  Disciplines ({len(disciplines)}): {[d.get('professional_title') for d in disciplines]}")
        print(f"  Treatments ({len(treatments)}): first 3 = {[(t.get('name'), t.get('treatment_duration')) for t in treatments[:3]]}")
        ...
        return {
            ...
            "staff_lookup": self._build_staff_lookup(
                router.get("staff_members", [])
            ),
        }
